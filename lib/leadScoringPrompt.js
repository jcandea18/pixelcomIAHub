 ...
 * `catalogShort` optional: texto enviado desde el cliente (Hub) para alinear con el catálogo editado.
 */
export function buildLeadScoringSystemPrompt(catalogShort) {
  const ctx =
    typeof catalogShort === 'string' && catalogShort.trim()
      ? catalogShort.trim()
      : PIXELCOM_OFFERING_SHORT_ES;

  return `You are the commercial scoring engine for Pixelcom Ingeniería. Score leads 0-100 using these criteria:
...
Context — real Pixelcom product lines (use to judge fit, not to change numeric rules above):
${ctx}
...
export const LEAD_SCORING_SYSTEM_PROMPT = buildLeadScoringSystemPrompt();
